...
flowchart LR
    ...
    subgraph stored["Stored results"]
        ...
        ART["CI artifacts<br/>(per-document detail; expires)"]
    end
    subgraph docs["Documents"]
        LIVE["benchmarks/README.md<br/>live block + findings"]
...
| --- | --- | --- |
| `knowledge/benchmarks/real-specimen-mrz-baseline.json` | provenance (`measured_on_ci_sha`, `measured_date`, `samples_data_sha`), `documents`, `scored`, `tier1_hits`, `tolerance`, `by_miss_kind{}`; `strict_names{}` once measured | CI only, via `tools/rebless.py` |
| `real-specimen-gate-report` (CI artifact) | `documents_detail[]` — per-document outcome, format, `ocr_ms`, retry telemetry | every gate run; **expires with the artifact** (no `retention-days` set) |
...
| Path | Role |
| --- | --- |
| `tools/rebless.py` | cohort PR → dispatch write-baseline → wait → download → classify (identical / non-scored / scored) → rewrite the live block for the first two; **stops for a human on a scored delta** |
...
| 2 | Zero false accepts: no checksum-valid MRZ returned for a document that carries none | `false_positive_mrz` in `REGRESSION_BUCKETS`; any non-zero fails the build | whole walked corpus (Observed — but as an absent key, see §6) |
...
1. **Field-level accuracy on real documents — names first.** The strict-name figure in the live
   block is *Derived* from a synthetic run; ADR-0013 forbids publishing the real figure until CI
   measures it, and only a minority of walked documents carry a fixture. Both are in flight
   (W1a, W2 of the current plan).
...
4. **Precision/recall framing.** `false_positive_mrz` is a count that must stay zero, never a rate
   over the refusal population. "Zero false accepts across N documents that could have produced
   one" is the sentence a KYC lead wants, and nothing generates it.
...
- **One 1,200-line file** holds the live block, the contract, the track catalogue and every dated
  finding. Every cohort, re-bless and finding appends to it; it is the hardest file to review and
  the likeliest to go stale.
...
- **Per-document evidence expires.** The baseline holds aggregates; `documents_detail` lives only in
  a CI artifact with the default retention, so every finding written from it becomes unverifiable
  after that window.
- **`false_positive_mrz` is absent, not zero**, in the committed JSON — right for gating, wrong for
  a downloaded artifact that should state the strongest claim explicitly.
...
| --- | --- | --- | --- | --- |
| 1 | Evidence that expires | CI writes `real-specimen-outcomes.jsonl` (stem, asset id, miss kind, format, `ocr_ms`, retry fields) beside the baseline; the baseline carries its sha256 | ~0.5 d | **W1a**, before the first re-bless with the new baseline shape |
| 2 | The strongest claim is invisible | serialize every bucket including zeros; add `refusal_population` so a report can print "0 false accepts across N refusal-class documents" | ~2 h | **W1a**, consumed by W4 |
...
| 5 | The 1,200-line file | split on the seam the tooling already cuts: `README.md` keeps the live block and the index; `METHODOLOGY.md` takes the contract and metric definitions; `FINDINGS.md` takes the dated log; anchors fixed by `check-doc-links.sh` | ~0.5 d | **W4**, right after PR-4.3 |
