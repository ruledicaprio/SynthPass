...
### 2026-09-26 — check-digit blind spots, exact: shared-shift pairs, composite alignment, and two blind confusables

**Derived**, closed form and exact integer enumeration
(`cargo run -p mrz --example checkdigit_blindspots_exact`), with the laws pinned against
`parse_td3` by `crates/mrz/tests/checkdigit_algebra.rs`. Landed from
[#504](https://github.com/ruledicaprio/SynthPass/issues/504) with the corrections its decision
asked for.

- **Same-Δ misreads cancel in closed form.** Two misreads that share a shift, such as two
  `0`→`O`, pass a document-number check digit at 9 of 36 cell pairs, the (weight-7, weight-3)
  ones, not the three adjacent pairs `blindspot.rs` named. A shared shift of 5, such as two
  `2`↔`7`, passes at all 36.
- **The composite is a second equation only for the dates.** For the document number and the TD3
  personal number it repeats the field's own weights. For a TD3 date of birth, k = 2 goes
  11.11% → 6.41%. Every weight is odd, so two digits over one field bottom out at 2%, not 1%.
- **Only `1`↔`L` and `6`↔`G` among `CONFUSABLES` share a residue**, the reverse of what
  `repair.rs` said. A verifying field that holds such a misread is accepted as read. A flag for
  it was declined: it would fire on 44 of 64 fixture document numbers and catches none of the
  five measured blind events.
- **Line-1 codes would be a detector the parser does not run.** One substitution of a listed
  code lands on another listed code 616 / 21,840 = 2.82% of the time (280 codes).
- **Enumeration verifies agreement, not soundness.** "Never a different accepted string" is false
  once the model holds a blind pair. Zero failures in 299 reads is a 99% lower bound at one-sided
  95%; the two-sided interval needs 368.
