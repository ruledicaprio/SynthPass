- **Two passport pages relabelled `_no_mrz` -> `_redacted_mrz`** -- the United Arab Emirates 2018
  ...
  absorbed by the redacted rung. `no_mrz_expected` 51 -> 49, `redacted_mrz` 37 -> 39, both outside
  the scored denominator, so no headline number moves. `samples/README.md` now spells out the
