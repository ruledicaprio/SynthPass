- **`MrzDate`, `Sex`, `RawDateField` and `DateRole`: typed date and sex values** (ADR-0019), not
  yet wired into `MrzData`.
  ...
  - `Sex` separates ICAO's three zone values (`M`, `F`, and `<` for unspecified) from a
    `NonConformant(char)` kept as read, where today's parser collapses every other character into
    `"X"`.
  - Their text form is a contract (ADR-0020): `Display`, `FromStr` and serde agree with the zone.
    ...
    checks for every field, role and pivot. Every value round-trips.
