...
| [synth_ab_diff.py](synth_ab_diff.py) | Compares two `synthpass-bench` JSON reports seed by seed (an A/B of two builds, or before/after one change). It classifies every moved seed (`wrong -> refused`, `refused -> correct`, ...) and prints each changed field's expected, before and after values. Run it manually after any synthetic A/B. Synthetic reports only: never point it at a real-specimen report. |
...
| [test_synth_ab_diff.py](test_synth_ab_diff.py) | Tests seed matching, move classification (including pre-#457 reports) and the not-an-A/B guard with literal report fragments, when changing the diff tool; run through the local unittest procedure below. |
