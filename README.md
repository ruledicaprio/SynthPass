...
- **144 / 155 = 92.9% on documents that can yield a hit** — how often extraction succeeds when
...
- **144 / 257 = 56.0% across the whole specimen corpus** — what happens if you point it at a pile
  of real documents. The 102-specimen gap is not failure: those carry no machine-readable zone at
...
- **The larger scored miss is now reading, not finding: `checksum_failed` (7 of 155)**, where a
  zone is found but fails a check digit, against `no_mrz_found` (4), where none is located at
